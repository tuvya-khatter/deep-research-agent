...
• **Workspace Detection** (always) → **Reverse Engineering** (brownfield only) → **Requirements Analysis** (always, adaptive depth) → **Conditional Phases** (as needed) → **Workflow Planning** (always) → **Code Generation** (always, per-unit) → **Grounding** (when the unit integrates any external contract) → **Build and Test** (always)
...
- Grounding: Verify generated code's assumptions about external contracts against non-model sources of truth (installed type stubs, recorded-real responses); replace model-authored mocks with reality fixtures. Blocks Build and Test until every boundary is grounded. (ALWAYS when the unit integrates any contract it does not own) — see construction/grounding.md
